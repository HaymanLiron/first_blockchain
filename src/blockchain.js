...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        console.log("Am i getting called even?");
        return new Promise((resolve) => {
            console.log("Liron Hayman is cool");
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if ((currentTime - requestTime) >= (5 * 60)) {
                reject(new Error('More than 5 minutes has elapsed.'));
            } else {
        ...
        return new Promise(async (resolve, reject) => {
            for (let block of self.chain) {
                if (await !block.validate()) {
                    ...
                    if (block.height > 0) { 
                        let prevBlock = self.chain.filter(prevBlockHeight => prevBlockHeight.height === block.height - 1)[0];
                        if (block.previousBlockHash !== prevBlock.hash) {
                            errorLog.push(new Error(`Invalid previousBlockHash at block #${block.height}`));
                        }
                    }                    
                }
            }
